...
public class WalkingBoss : LivingEntity
{
    ...
    private IEnumerator SkulSummon()
    {
        while(true)
        {

            Walker redSkul = Instantiate(redSkulPrefab, transform.position+transform.forward*2,Quaternion.identity);
            redSkul.onDeath +=() =>SkulReborn(redSkul,enemySpawner.redSkulMiniPrefab);          
            ParticleSystem spawn1 = Instantiate(spawnParticle,transform.position+transform.forward*2,Quaternion.identity);
            yield return new WaitForSeconds(0.5f);

            ShootingEnemy blueSkul = Instantiate(blueSkulPrefab,transform.position+transform.forward*-2,Quaternion.identity);
            blueSkul.Setup(2000,5.2f);
            blueSkul.onDeath+=()=>SkulRebornBlue(blueSkul,enemySpawner.blueSkulMiniPrefab);
            ParticleSystem spawn2 = Instantiate(spawnParticle,transform.position+transform.forward*-2,Quaternion.identity);
